...
@section("content")
    <div id="title-breadcrumb-option-demo" class="page-title-breadcrumb">
        <div class="page-header pull-left">
            <div class="page-title">Edit Role And Permission-> {{ $roleName->name  }}</div>
        </div>
            ...
            <div class="col-lg-12">
                <div class="panel panel-yellow">
                    ...
                    <div class="panel-body pan">
                        <form action={{route("role.update", $roleName->id)}} method="post" class="horizontal-form" enctype="multipart/form-data">
                            @csrf
                            <div class="form-body pal">
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <input name="name" value={{ $roleName->name  }} type="text"  class="form-control"/>
                                        </div>
                                    </div>
                                  </div>
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="form-group">
                                                <label>
                                                    <input type="checkbox" tabindex="7" id="checkAllCheckBox"
                                                        {{ App\User::RoleHasPermission($roleName, $all_permissions)? "checked":"" }}/>All
                                                </label>
                                            <hr>
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-md-3">
                                        <div class="form-group">
                                            @php
                                               ...
                                               $j =1;
                                            @endphp
                                                    <label><input type="checkbox" tabindex="5" name="groupPermission[]" id="management-{{ $i }}"
                                                                  onclick="CheckPermissionByGroup('role-{{$i}}-management-checkbox', this)"
                                                            {{ App\User::RoleHasPermission($roleName, $permissions)? "checked":"" }}   />&nbsp;{{$group->group_name}}</label>
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-md-9 role-{{$i}}-management-checkbox">
                                        <div class="form-group">
                                            ...
                                            @foreach($permissions as $per)
                                                <div class="checkbox">
                                                    <label>
                                                        <input tabindex="5" name="permission[]"
                                                                  {{ $roleName->hasPermissionTo($per->name)? "checked":""  }}
                                                                  onclick="checkSingleRole('role-{{$i}}-management-checkbox', 'management-{{ $i }}', {{count($permissions)}})"
                                                                  value={{$per->name}} type="checkbox"/>&nbsp;{{$per->name}}</label>
                                                </div>
                                                @php $j++ @endphp
                                            @endforeach
                                        </div>
